...
#v-itemCard
  ...
  .v-itemcard__content
    ...
    .v-itemcard__images
      .v-itemcard__images__discount(v-if="item.discount_price")
        span.v-itemcard__images__discount__countdiscount {{countdiscount()}}
    ...
    .v-itemcard__info
      .v-itemcard__prices
        ...
        .v-itemcard__prices-title Цена&nbsp;{{ item.unit.toLowerCase() }}
        .v-itemcard__prices-values
          .v-itemcard__prices-clubCost(v-if="!item.discount_price") клубная:&nbsp;
            span.clubCost-value(v-if="item.club_cost") {{ item.club_cost }} &#8381;
          .v-itemcard__prices-clubCost.strike(v-else) розница:&nbsp;
            span.cost-value {{ item.cost }} &#8381;
          .v-itemcard__prices-cost(v-if="!item.discount_price" :class="{'strike': item.discount_price}") розница:&nbsp;
            span.cost-value {{ item.cost }} &#8381;
          .v-itemcard__prices-cost(v-if="item.discount_price")
  ...
  .v-itemcard__buttons--mobile
    ...
    .v-itemcard__costs--mobile
      .v-itemcard__prices-clubCost(v-if="!item.discount_price") клубная:&nbsp;
        span.clubCost-value.ruble(v-if="item.club_cost") {{ item.club_cost }} &#8381; {{ item.unit.toLowerCase() }}
      .v-itemcard__prices-cost(:class="{'strike': item.discount_price}") розница:&nbsp;
        span.cost-value.ruble {{ item.cost }} &#8381; {{ item.unit.toLowerCase() }}
      .v-itemcard__prices-cost(v-if="item.discount_price")
        .v-itemcard__prices-discount скидка:&nbsp;
          span.discount-value {{item.discount_price }} &#x20bd; {{ item.unit.toLowerCase() }} 
  ...
  .v-itemcard__summary-align.mobile
    ...
    .v-itemcard__summary-wrapper
      h6.v-itemcard__summary-title Итого:&nbsp;
      span.v-itemcard__summary-value.ruble {{ summary }} &#x20bd;
  .v-itemcard__reccomend(v-if="same.length")
  ...
  .v-itemcard__reccomend(v-if="buyed.length")
    h3 С этим покупают
      img(:src='require("@/assets/images/divider.png")')
    vItem( v-for='byItem in additionalProducts.currentProductBuyedItems', :key='byItem.id', :item='byItem')
  ...
  .v-itemcard__content.bg-gray.recommended(v-if="same.length || buyed.length")
...
export default {
  ...
  async asyncData({ $axios, params, store, route, redirect }) {
    let product
    const tmpProduct = await $axios.$get(`/api/getProduct/${params.id}${params.region ? `?region=${params.region}` : ""}`)
    
    if(!tmpProduct) return redirect("/404")
    ...
    if(params.item) {
      product.counter = params.item.counter
    }
  ...
  computed: {
    ...mapGetters({
      ...
      localDataInited: 'GET_LOCAL_DATA_INITED',
    }),
    ...
    breadcrumbs() {
      return [
        ...
        ...this.getCategoryBreadCrumbs(true),
        {
  ...
  methods: {
  ...mapActions({watchedItems: 'product/setWatchedItems'}),
  ...
  mounted() {
    ...
    let timer = setInterval(() => {
      if(this.localDataInited) {
        this.watchedItems({ ...item, type: 'watched', region: this.REGION.value })
        clearInterval(timer)
      }
    }, 100)
  },
...
div.v-itemcard__prices-clubCost.strike
  color: black
div.v-itemcard__costs--mobile > div:nth-child(2) > div
  @include phones
    margin-bottom: 0
    margin-top: 5px
</style>
