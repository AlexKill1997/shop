<template lang="pug">
  .v-catalog__filters(v-if="Object.keys(PRODUCT_FILTERS).length")
    .catalog__filters__header(@click="$emit('toggle', !parentShow)")
      h4.catalog__filters__heading Фильтр товаров:

      img.catalog__icon(:class="{'turnHalf':parentShow}" :src="require('@/assets/images/items/arrow-circle1.svg')" title="Развернуть")
    ul.filters__list(:class="{'appear': parentShow }")
      li.filters__list-item(v-for="(filter, name, index) of PRODUCT_FILTERS" v-if="show ? index < 100 : index < 28 " :key="name")
        .filters__header(
          ...
          p.filters__heading(:class="{'filters__heading__notwebkit': filter.opened}") {{ name }}
    ...
    .categories__buttons(:class="{'appear__butt':parentShow }")
