<template lang="pug">
  .itemHorizontalnoPic__container
    .itemHorizontalnoPic__container__discount(v-if="item.discount_price")
      img(:src="require('@/assets/images/icons/discount.svg')")
      span.itemHorizontalnoPic__container__discount__countdiscount {{countdiscount()}}
    nuxt-link(:to="{name: 'products', params: { id: item.slug, item: item }}")
      .itemHorizontalnoPic__title
        h3.itemHorizontalnoPic__heading {{ item.title }}
        .itemHorizontalnoPic__article
          span Артикул:&nbsp;
          span.article-value {{ item.article }}

    .itemHorizontalnoPic__prices
      .itemHorizontalnoPic__clubCost(v-if="item.club_cost && !item.discount_price")
        span.itemHorizontalnoPic__clubCost-title Клубная:&nbsp;
          span.itemHorizontalnoPic__clubCost-value.ruble {{ item.club_cost }} &#8381;
          |  {{ item.unit.toLowerCase() }}
      .itemHorizontalnoPic__cost(:class="{'strike': item.discount_price}")
        span.itemHorizontalnoPic__cost-title Розница:&nbsp;
          span.itemHorizontalnoPic__cost-value.ruble {{ item.cost }} &#8381;
          |  {{ item.unit.toLowerCase() }}
      .itemHorizontalnoPic__discount(v-if="item.discount_price")
        span.itemHorizontalnoPic__discount-title Скидка: &nbsp;
          span.itemHorizontalnoPic__discount-value.ruble {{item.discount_price}} &#8381;
          |  {{ item.unit.toLowerCase() }}
    .itemHorizontalnoPic__shopping
      template(v-if="!isInCart(item)")
        .itemHorizontalnoPic__counter
          .itemHorizontalnoPic__counter-wrapper
            button.decrease(type="button" @click="changeProductCount('decrement')") –
            input.itemHorizontalnoPic__quantity(
              type="number",
              name="quantity",
          ...
          .itemHorizontalnoPic__add-to-cart(@click="addToCart")
        .itemHorizontalnoPic__wrap
          .itemHorizontalnoPic__one-click-buy(@click="orderClick()" :key="item.id")
            img.one-click-buy__icon(:src="require('../assets/images/icons/cursor.svg')")
            span.one-click-buy__title Заказать в 1 клик
          .itemHorizontalnoPic__favorite(@click="clickFavoritesHandler")
...
.itemHorizontalnoPic
  ...
  &__clubCost-title, &__cost-title, &__discount-title
    font-size: 18px
    line-height: 30px
    letter-spacing: .07em
  ...
  &__clubCost-value, &__cost-value, &__discount-value
    font-size: 24px
    font-weight: 700
