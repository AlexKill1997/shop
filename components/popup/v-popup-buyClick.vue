<template lang="pug">
.v-popup
  .v-popup-wrapper(@click="closePopup")
  .v-popup__tnank
    .v-popup__tnank--header
      ...
      .v-popup__tnank--header-descWrapper
        p.v-popup__tnank--header-title Вы успешно купили товар
      img.v-popup__tnank--header-closeIcon(
          :src="require('../../assets/images/icons/close_mark.svg')"
          title="Закрыть окно"
          alt="Закрыть"
          @click="closePopup"
      )
    .v-popup__content__thank
      p На Ваш номер телефона был отправлен пароль для входа в Личный кабинет!
      p наш менеджер свяжется с вами в течении 5 минут!
      nuxt-link.success(to="/account" @click="closePopup") Перейти в личный кабинет
</template>
