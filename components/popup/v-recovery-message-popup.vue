<template lang="pug">
  vBasePopup(
    ...
    template(v-slot:header)
      ...
      .v-popup__tnank--header-descWrapper
        p.v-popup__tnank--header-title Ваш пароль востановлен!
      img.v-popup__reminder-closeIcon(
    ...
    template
      p.remind__password На Ваш номер телефона был отправлен пароль для входа в Личный кабинет!

      //- nuxt-link.success(v-if="user" to="/account" @click="closePopup") Перейти в личный кабинет
</template>

<script>
import popupMixin from '@/components/popup/mixins/popupMixin'
import thanksPopupMixin from '@/components/popup/mixins/thanksPopupMixin'

export default {
  name: "ConsumptionThanksPopup",
  mixins: [popupMixin, thanksPopupMixin]
}
</script>
...
.remind__password
  text-align: center
  font-size: 14px
.v-popup__reminder-closeIcon
  position: absolute
  top: 4px
  right: 4px
  cursor: pointer

#v-popup > div.v-popup > div.v-popup__header > svg
  @include phones
    display: none
</style>
